...
{
    public class MineFightResultPanel : FightResultBasePanel
    {
        ...
        public override void Init()
        {
            ...
            if (view.isWin)
            {
                int income = Logic.UI.Mine.Controller.MineController.instance.inCome;
                if (income > 0)
                {
                    text_baseRes_des.text = Localization.Get("ui.fightResultView.text_total_money_des");
                    text_baseRes.transform.GetComponent<NumberIncreaseAction>().Init(0, income);
                }
            }
            InitShowComponent();
            InitClickEvent();
        ...
        {
            FightResultProxy.instance.GotoMainScene(type, FightResultProxy.instance.QuitMineCallback);
        }
        #region click event
        protected virtual void ClickBackBtnHandler()
        {
            FightResultQuit(FightResultQuitType.GO_MINE);
        }
