...
{
	public class PvpFightResultPanel : FightResultBasePanel 
	{
		...
		public override void Init()
		{
			...
			if(view.isWin)
			{
				text_baseRes_des.text = Localization.Get("ui.fightResultView.text_honor_des");
				text_baseRes.transform.GetComponent<NumberIncreaseAction>().Init(0, GlobalData.GetGlobalData().challengeSuccPrize);
			}else
			{
				Transform tranFailedBaseRes = view.lostRoot.transform.Find("lost_extra_root2/honor_root/text_total_honor");
				tranFailedBaseRes.parent.gameObject.SetActive(true);
				tranFailedBaseRes.GetComponent<NumberIncreaseAction>().Init(0, GlobalData.GetGlobalData().challengeFailPrize);
			}
			InitShowComponent();
			InitClickEvent();
		}
		...
		{
			FightResultProxy.instance.GotoMainScene(type, FightResultProxy.instance.QuitPvpCallback);
        }
		#region click event
		protected virtual void ClickBackBtnHandler()
		{
			FightResultQuit(FightResultQuitType.Go_Pvp);
		}
