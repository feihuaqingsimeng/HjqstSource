...
{
	public class ExpeditionFightResultPanel : FightResultBasePanel 
	{
		...
		public override void Init()
		{
			...
			if(view.isWin)
			{
				text_baseRes_des.text = Localization.Get("ui.expedition_fightResultView.text_expedition_point_des");
				text_baseRes.text = FightResultProxy.instance.dropBaseResDictionary.GetValue(BaseResType.ExpeditionPoint).ToString();
			}
			InitShowComponent();
			InitClickEvent();
		}
		...
		{
			FightResultProxy.instance.GotoMainScene(type, FightResultProxy.instance.QuitExpeditionCallback);
        }
		#region click event
		protected virtual void ClickBackBtnHandler()
		{
			FightResultQuit(FightResultQuitType.Go_Expedition);
		}
