...
namespace PathologicalGames
{
    ...
    ///    which enabled shorter names to reduces the length of lines of 
    ...
    public class SpawnPoolsDict : IDictionary<string, SpawnPool>
    {
        ...
        private bool assertValidPoolName(string poolName)
        {
            ...
            {
                Debugger.Log(string.Format("A pool with the name '{0}' already exists",
                                        poolName));
                return false;
            }
        ...
        internal void Add(SpawnPool spawnPool)
        {
            ...
            {
                Debugger.LogError(string.Format("A pool with the name '{0}' already exists. " +
        ...
        internal bool Remove(SpawnPool spawnPool)
        {
            if (!this.ContainsKey(spawnPool.poolName))
            {
                Debugger.LogError(string.Format("PoolManager: Unable to remove '{0}'. " +
