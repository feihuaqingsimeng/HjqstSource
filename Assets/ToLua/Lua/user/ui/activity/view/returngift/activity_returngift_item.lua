local t={}
t.__index = t
local name='activity_returngift_item'
...
function t:Start(transform, info)
  if nil == self.transform then 
    ...
    self._TextlevelValue = self.transform:FindChild('descText'):GetComponent(typeof(Text))
...
function t:Init(info)
  ...
  self._TextlevelValue.text = string.format(LocalizationController.instance:Get("event_1006_Days"), tonumber(itemData.event_param))
  self:InitAwards()
  self:InitButtons()
...
function t:InitButtons()
  ...
  self._BtnAward.gameObject:SetActive(self.info.IsJoin)
end
