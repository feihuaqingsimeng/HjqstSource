...
local name = 'activity_consume_view'
...
local activity_consume_item = require("ui/activity/view/accumulateconsume/activity_consume_item")
...
function t:OnItemInit(go, index)
  ...
  if nil == itemdata then do return end end
   ...
   if self.m_aryScrollItem[go] == nil then
      self.m_aryScrollItem[go]= activity_consume_item.New(go.transform)
