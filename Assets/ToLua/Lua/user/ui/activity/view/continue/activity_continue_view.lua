...
local name = 'activity_levelup_view'
...
local activity_continue_item = require("ui/activity/view/continue/activity_continue_item")
...
function t:Start(btype)
  ...
  self._Text1 = self.transform:FindChild('left/titletime'):GetComponent(typeof(Text))
  self._Text2 = self.transform:FindChild('left/titledesc'):GetComponent(typeof(Text))
  self._Text3 = self.transform:FindChild('left/titledesc/desc'):GetComponent(typeof(Text))
  self._Text4 = self.transform:FindChild('left/titletime/time'):GetComponent(typeof(Text))  
...
function t:OnItemInit(go, index)
  ...
  if nil == itemdata then do return end end
   ...
   if self.m_aryScrollItem[go] == nil then
      self.m_aryScrollItem[go]= activity_continue_item.New(go.transform)
