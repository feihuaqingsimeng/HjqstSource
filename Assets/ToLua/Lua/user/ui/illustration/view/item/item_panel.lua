...
function t.InitComponent()
  ...
  local goEquipPrefab = t.transform:Find('Scroll View/illustratedButton').gameObject
  goEquipPrefab:SetActive(false)
  t.goItemPrefab = t.transform:Find('Scroll View/illustratedItemButton').gameObject
  t.goItemPrefab:SetActive(false)
  --info
  ...
  t.tranAttrRoot.gameObject:SetActive(false)
  --path
  t.goodsPathButtons = {}
  for i = 0,2 do
...
function t.RefreshInformation()
  t.textName.text = LocalizationController.instance:Get(t.selectIllustrationInfo.itemInfo.itemData.name)
  t.textDes.text = LocalizationController.instance:Get(t.selectIllustrationInfo.itemInfo.itemData.des)
  if t.selectIcon == nil then
    t.selectIcon = common_item_icon.New(t.tranIconRoot)
  end
  t.selectIcon:SetItemInfo(t.selectIllustrationInfo.itemInfo)
  --path
  local pathList = LuaCsTransfer.GetDropGoodsPathDatas(BaseResType.Item,t.selectIllustrationInfo.id,0,0)
  local length  = 0
  ...
  elseif pathList.Length == 0 then
    ...
    local resData = game_res_data.New(BaseResType.Item,t.selectIllustrationInfo.id,0,0)
...
function t.onResetItemHandler(go,index)
  ...
  if not item then
    item = illustration_content_view.BindTransform(go.transform,t.goItemPrefab)
  ...
  item:SetData(scrollItemData.titleStr,illustrationList,IllustrationType.item)
...
function t.OnClickItemBtnHandler(illustrationInfo)
  ...
  print(t.selectIllustrationInfo.itemInfo.itemData.id)
