...
function attackedBuff_605(character,target,skillInfo, judgeType, actualHPValue)
    ...
    if(num == nil) then		
      ...
      if(target.HP < target.maxHP * lowRate and target.HP > 0)  then
        ...
        local physicAttack = 0.3  --物理攻击比例
        target:AddBuff(character,target,skillInfo,mechanics,Logic.Enums.BuffType.PhysicsAttack,Logic.Enums.SkillLevelBuffAddType.None,Logic.Enums.BuffAddType.Percent,lastTime,physicAttack,1,1,false,true)
      end
    end
end